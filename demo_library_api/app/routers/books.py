...
async def listBooks(limit: int = Query(20, ge=1, le=100), offset: int = Query(0, ge=0)):
    items = db.list_all("books", limit=limit, offset=offset)
    total = len(db.get_table("books"))
    return {"items": items, "total": total, "limit": limit, "offset": offset}

@router.post("/books", status_code=201)
async def createBook(payload: BookCreate, authorization: Optional[str] = Header(None)):
...
async def updateBook(book_id: str, payload: BookUpdate, authorization: Optional[str] = Header(None)):
    if not authorization or not authorization.startswith("Bearer "):
        raise HTTPException(status_code=401, detail="Missing or invalid bearer token")
...
async def deleteBook(book_id: str, authorization: Optional[str] = Header(None)):
    if not authorization or not authorization.startswith("Bearer "):
        raise HTTPException(status_code=401, detail="Missing or invalid bearer token")
